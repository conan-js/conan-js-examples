...
class FlowThread {
    ...
    requestStatus(statusLike, isStep) {
        ...
        let tracker = this.flowOrchestrator.createRuntimeTracker(this.flowController, FlowEventSource.FLOW_THREAD, exports.FlowEventType.REQUESTING_STATUS, statusLike).start(`${status.name}`);
    ...
    requestTransition(transition, isStep) {
        ...
        let tracker = this.flowOrchestrator.createRuntimeTracker(this.flowController, FlowEventSource.FLOW_THREAD, exports.FlowEventType.REQUESTING_TRANSITION, transition).start(`${transition.transitionName}`);
    ...
    processStateAndReactions(statusRequest, isStep) {
        let tracker = this.flowOrchestrator.createRuntimeTracker(this.flowController, FlowEventSource.FLOW_THREAD, exports.FlowEventType.PROCESSING_STATUS).start(`${statusRequest.status.name}`);
    ...
    flagAsSettled(statusRequest, isStep) {
        let tracker = this.flowOrchestrator.createRuntimeTracker(this.flowController, FlowEventSource.FLOW_THREAD, exports.FlowEventType.SETTLING_STATUS).start(`${statusRequest.status.name}`);
    ...
    onStateRequestCompleted(stateMachineRequest, queuedReactions, queuedStatuses, queuedStates, queuedTransitions, queuedSteps) {
        let tracker = this.flowOrchestrator.createRuntimeTracker(this.flowController, FlowEventSource.FLOW_THREAD, exports.FlowEventType.STATUS_REQUEST_COMPLETED).start(`${stateMachineRequest.status.name}`);
        let rootRequestTracker = this.flowOrchestrator.createRuntimeTracker(this.flowController, FlowEventSource.FLOW_THREAD, exports.FlowEventType.ROOT_REQUEST);
...
class FlowEventsTracker {
    ...
    settleProcessingStatus(statusRequest, isStep) {
        if (!isStep) {
            ...
            this.lastEvent.type = exports.StatusEventType.STATUS;
            ...
            this.lastEvent.type = exports.StatusEventType.STATE;
    ...
    process(lastEvent) {
        ...
        if (lastEvent.type === exports.StatusEventType.STATE || lastEvent.type === exports.StatusEventType.PROCESSING_STATE) {
    ...
    serializeStatesWithStatus(filterOptions) {
        return this.serializeWithStatusInfo(Object.assign(Object.assign({}, filterOptions), { eventTypes: [exports.StatusEventType.STATE] }));
    }
    serializeStates(filterOptions) {
        return this.serializeWithStatusInfo(Object.assign(Object.assign({}, filterOptions), { eventTypes: [exports.StatusEventType.STATE] })).map(it => it.event);
    }
    serializeStatuses(filterOptions) {
        return this.serializeWithStatusInfo(Object.assign(Object.assign({}, filterOptions), { eventTypes: [exports.StatusEventType.STATUS] })).map(it => it.event);
    }
...
class MutatorsFactory {
    static createDefaultStepsDef(flowAnchor) {
        return (getData) => ({
            $update(reducer) {
                ...
                let tracker = flowThread.flowOrchestrator.createRuntimeTracker(flowThread.flowController, FlowEventSource.CONTEXT, exports.FlowEventType.USER_CODE, finalValue).start();
    ...
    static createDefaultTransitionDef(flowAnchor) {
        return (() => ({
            $toStatus(toStatus) {
                ...
                let flowThread = flowAnchor.currentThread.flowThread;
                flowThread.flowOrchestrator.createRuntimeTracker(flowThread.flowController, FlowEventSource.CONTEXT, exports.FlowEventType.USER_CODE, status).start().debug(`$toStatus`, status).end();
                return status;
            }
        }));
    }
    ...
    static bindToAnchor(expectedStatusName, flowAnchor, toBind, type) {
        return Proxyfier.proxy(toBind, (originalCall, metadata) => {
            ...
            let tracker = flowAnchor.currentThread.flowThread.flowOrchestrator.createRuntimeTracker(flowController, FlowEventSource.CONTEXT, exports.FlowEventType.USER_REACTIONS, flowController.flowDef.nature).start();
            ...
            try {
                ...
                flowAnchor.createRuntimeTracker(exports.FlowEventType.ERROR_USER_CODE, e).start().milestone(`error running ${metadata.methodName}`, e).end();
                return undefined;
            }
        });
    }
...
class FlowImpl {
    ...
    start(initialStatus) {
        let tracker = this.flowOrchestrator.createRuntimeTracker(this, FlowEventSource.FLOW_CONTROLLER, exports.FlowEventType.STARTING, initialStatus).start();
        if (this.isRunning) {
    ...
    onceOn(stateName, def, name) {
        let tracker = this.flowOrchestrator.createRuntimeTracker(this, FlowEventSource.FLOW_CONTROLLER, exports.FlowEventType.ONCE_ON, stateName).start();
    ...
    addReaction(statusName, reaction) {
        let tracker = this.flowOrchestrator.createRuntimeTracker(this, FlowEventSource.FLOW_CONTROLLER, exports.FlowEventType.ADDING_REACTION).start();
    ...
    processReactions(statusName) {
        let tracker = this.flowOrchestrator.createRuntimeTracker(this, FlowEventSource.FLOW_CONTROLLER, exports.FlowEventType.PROCESSING_REACTIONS).start();
    ...
    runIf(statusName, reaction, elseIf) {
        let tracker = this.flowOrchestrator.createRuntimeTracker(this, FlowEventSource.FLOW_CONTROLLER, exports.FlowEventType.RUN_IF, reaction.name).start();
        if (!this.isRunning) {
            if (elseIf)
                elseIf();
            tracker.end();
    ...
    doRun(reaction, currentState) {
        let tracker = this.flowOrchestrator.createRuntimeTracker(this, FlowEventSource.FLOW_CONTROLLER, exports.FlowEventType.RUN, reaction.name).start();
    ...
    deferInto(statusFrom, statusTo, mutatorsCbAsapLike) {
        ...
        let tracker = this.flowOrchestrator.createRuntimeTracker(this, FlowEventSource.FLOW_CONTROLLER, exports.FlowEventType.MONITORING, {
            asap: defer.action,
            name: defer.name,
    ...
    log(msg) {
        let tracker = this.flowOrchestrator.createRuntimeTracker(this, FlowEventSource.USER_MSG, exports.FlowEventType.USER_CODE, msg).start();
...
class Flows {
    ...
    static createController(flowDef) {
        ...
        let tracker = flowOrchestrator.createRuntimeTracker(flowImpl, FlowEventSource.FLOW_FACTORY, exports.FlowEventType.CREATING, flowDef).start();
        tracker.end();
        return flowImpl;
    }
}
...
class MonitorImpl {
    ...
    openTransaction(name) {
        if (this.currentTransaction == null) {
            this.currentTransaction = this.mainThread.createRuntimeTracker(exports.FlowEventType.MONITOR_TRANSACTION).start();
...
class Monitors {
    ...
    static orchestrate(event, asyncThread, metaFlow) {
        ...
        if (event.type === exports.FlowEventType.ERROR_USER_CODE) {
            metaFlow.assertOn(metaFlow.getCurrentStatusName(), onAny => onAny.do.toError());
        }
        if (event.type === exports.FlowEventType.MONITORING) {
        ...
        if (event.type === exports.FlowEventType.MONITOR_TRANSACTION && (timing === FlowEventTiming.START || timing === FlowEventTiming.END)) {
        ...
        if (event.type === exports.FlowEventType.ROOT_REQUEST) {
...
exports.Lists = Lists;
exports.LoggerFilters = LoggerFilters;
...
exports.getLoggingFilter = getLoggingFilter;
exports.isAsap = isAsap;
exports.setLoggingFilter = setLoggingFilter;
exports.stateConnect = stateConnect;
exports.stateLive = stateLive;
exports.stateMapConnect = stateMapConnect;
exports.updateLoggingFilter = updateLoggingFilter;
