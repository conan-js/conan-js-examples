...
class FlowThread {
    ...
    flagAsSettled(statusRequest, isStep) {
        let tracker = this.flowOrchestrator.createRuntimeTracker(this.flowController, FlowEventSource.FLOW_THREAD, FlowEventType.SETTLING_STATUS, statusRequest).start(`${statusRequest.status.name}`);
    ...
    createContext(statusLike, doChain) {
        ...
        return {
            ...
            log: (msg) => {
                let tracker = this.flowOrchestrator.createRuntimeTracker(this.flowController, FlowEventSource.USER_MSG, FlowEventType.USER_CODE, msg).start();
...
class FlowImpl {
    ...
    addReaction(statusName, reaction) {
        let tracker = this.flowOrchestrator.createRuntimeTracker(this, FlowEventSource.FLOW_CONTROLLER, FlowEventType.ADDING_REACTION, reaction.name).start();
    ...
    processReactions(statusName) {
        let tracker = this.flowOrchestrator.createRuntimeTracker(this, FlowEventSource.FLOW_CONTROLLER, FlowEventType.PROCESSING_REACTIONS, statusName).start();
    ...
    chainInto(statusFrom, statusTo, mutatorsCb) {
        const [next, asap] = Asaps.next(`chain[${this.getName()}]-[${statusFrom}->${statusTo}]`, FlowEventNature.AUX);
        if (this.getCurrentStatusName() !== '$init') {
            ...
            this.onceOn(`$init`, () => mutatorsCb(statusToDef.steps));
        }
        return asap;
    }
